...
public class StudyRepo extends SimpleJpaRepository<FullStudyEntity, String> implements JpaRepository<FullStudyEntity, String>{
  ...
  public StudyRepo(EntityManager em) {
    super(FullStudyEntity.class, em);
    this.em = em;
  }

  public Optional<FullStudyEntity> readStudyWithSamples(String studyId, boolean includeInfo){
    val graph = em.createEntityGraph(STUDY_WITH_SAMPLES_PATH);
    val hints = new HashMap();
    hints.put("javax.persistence.fetchgraph", graph);
    val s = em.find(FullStudyEntity.class, studyId, hints);
    em.detach(s);
